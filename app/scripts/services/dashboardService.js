 ...
 app.factory('DashboardService', ['$q', '$http', '$rootScope', function ($q, $http, $rootScope)
     {
         ...
         dashboardService.getDashboardDetails = function () {
             ...
             $http({
                 method: 'GET'
                 , url: 'http://104.214.115.118:8085/process_query?order_id='
         ...
         dashboardService.getChainDetails = function (num) {
             ...
             $http({
                 method: 'GET'
                 , url: 'http://104.214.115.118:8089/getBLockDetails?blocknumber=' + num
         ...
         dashboardService.getStartFlag = function () {
             var deferred = $q.defer();
             $http({
                 method: 'GET', 
                 url: 'http://104.214.115.118:4000/fileID/'
             }).then(function (chainDetails) {
                 deferred.resolve(chainDetails);
             }, function (error) {
                 deferred.reject(error);
         ...
         dashboardService.insertFlagData = function (insData) {
             var deferred = $q.defer();
             $http({
                 method: 'POST'
                 , url: 'http://104.214.115.118:4000/updateID'
                 , data: insData
                 , json: true, //send the desired json data in the post....
                 headers: {
                     'Content-Type': 'application/json'
